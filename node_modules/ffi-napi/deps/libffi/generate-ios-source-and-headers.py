...
def sdkinfo(sdkname):
    ...
    for line in subprocess.Popen(
        ["xcodebuild", "-sdk", sdkname, "-version"], stdout=subprocess.PIPE
    ).stdout:
...
def latest_sdks():
    ...
    latest_device = None
    for line in subprocess.Popen(
        ["xcodebuild", "-showsdks"], stdout=subprocess.PIPE
    ).stdout:
...
def move_source_tree(
    src_dir, dest_dir, dest_include_dir, arch=None, prefix=None, suffix=None
):
...
def build_target(platform):
    def xcrun_cmd(cmd):
        return subprocess.check_output(
            ["xcrun", "-sdk", platform.sdkroot, "-find", cmd]
        ).strip()
    ...
    if not os.path.exists(build_dir):
        ...
        env = dict(
            CC=xcrun_cmd("clang"),
            LD=xcrun_cmd("ld"),
            CFLAGS="-arch %s -isysroot %s -miphoneos-version-min=4.0"
            % (platform.arch, platform.sdkroot),
        )
